...
> `flipline/proto/shirt-test.html` containing:
>
...
> - A **headless harness** (`node shirt-harness.js`) that drives the real update loop and asserts the
>   feel budget in frames, so an agent can prove a change without a human playing.
>
> ****Status here: the lab EXISTS** (`proto/shirt-test.html`, commit `3095ca4`) - one shirt every 3s on a fixed clock, deflect instead of die. This is the game the method came from. Its cockpit is thin: no live tuning keys, no NUMBERS DUMP, no freeze/frame-step, no headless harness of its own (the main build's `?bot=1` harness is separate). Bring it up to the standard above before the roadmap resumes. For a dodger the 'one enemy' is one obstacle type, and the ladder is the same: each new garment/hazard must ask a different question, not the same gap at a higher speed.**
>
