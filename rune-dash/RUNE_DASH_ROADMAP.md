...
> `rune-dash/proto/freeze-jump-test.html` containing:
>
...
> - A **headless harness** (`node freeze-jump-harness.js`) that drives the real update loop and asserts the
>   feel budget in frames, so an agent can prove a change without a human playing.
>
> ****Status here: NO LAB YET,** and this game is paused - which makes it the cheapest one to restart correctly. The core verb is the freeze-jump. Build the lab for **one player character against one hazard**, prove the freeze-jump is fun in isolation, and only then unpause the backlog below.**
>
