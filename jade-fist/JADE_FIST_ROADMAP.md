...
> `jade-fist/proto/duel-test.html` containing:
>
...
> - A **headless harness** (`node duel-harness.js`) that drives the real update loop and asserts the
>   feel budget in frames, so an agent can prove a change without a human playing.
>
> ****Status here: the lab EXISTS** (`proto/duel-test.html`, JF-#065/#066, live-verified, harness 10/10) and it already found two real bugs the full game hid: the dodge->counter loop did not close one-on-one, and the telegraph ran at nearly twice human choice-reaction time. Its cockpit is still partial - it has live tuning keys and a HUD readout, but **no NUMBERS DUMP, no freeze/frame-step, no slow-mo key, no overlay**. Add those, then Mike feel-gates (steps DL-a..DL-i in the dev notes). Nothing goes back into `index.html` until he does. See `HANDOFF-jade-fist-duel-lab.md`.**
>
